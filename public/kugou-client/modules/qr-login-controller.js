...
export function createQrLoginController({ userService, auth, ui, scheduler }) {
  return {
    ...
    async start() {
      this.stop(true);
      const keyPayload = await userService.getQrKey();
      const key = keyPayload.qrcode || keyPayload.key || '';
      if (!key) throw new Error('二维码 key 获取失败');

      const qrPayload = await userService.createQrCode(key, true);
      this.state = {
        key,
        image: qrPayload.base64 || ui.defaultCover(),
        url: qrPayload.url || '',
        status: 1,
        statusText: '等待扫码'
      };
    ...
    async poll() {
      ...
      const payload = await userService.checkQrStatus(this.state.key);
