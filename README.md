...
This repository includes a browser client in `public/kugou-client` for login, search, playlist browsing, playback, lyrics, cover art, and MV playback.

### Access

After starting the API service, open:

```text
http://127.0.0.1:3000/kugou-client/index.html
```

For deployed environments, replace `127.0.0.1:3000` with your own host.

### Client Defaults

- Platform: `life`
- Raw upstream calls: `/raw-api`
- Stable gateway calls: `/api`

### Client Docs

- Repo doc: `docs/kugou-client-api.md`
- Page doc: `public/kugou-client/docs/api.md`
...
Then open `http://127.0.0.1:3000/kugou-client/index.html` in your browser.
...
![](https://img.shields.io/github/repo-size/MakcRe/KuGouMusicApi?style=for-the-badge&label=files&color=cf8ef4&labelColor=373e4d "文件大小")
...
```text
$ 复制 .env.example 为 .env，并把里面的 `platform=''` 改为 `platform=lite`
$ 注意不同版本的平台 token 不通用。
```
...
5. 直接点 `Continue`
6. 若需要部署版本为概念版（不需要该步骤可跳过），在 `Environment Variables` 添加 `key` 为 `platform`，`Value (Will Be Encrypted)` 为 `lite` 然后点击 `Add`
